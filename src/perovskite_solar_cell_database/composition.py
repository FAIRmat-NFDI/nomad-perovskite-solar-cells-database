...
class PerovskiteIon(PureSubstance, EntryData):
    """
...
class IonComponent(SystemComponent):
    abbreviation = Quantity(
        type=str,
        description='The standard abbreviation of the ion. If the abbreviation is in the archive, additional data is complemented automatically',
        a_eln=ELNAnnotation(component=ELNComponentEnum.StringEditQuantity),
...
class PerovskiteComposition(CompositeSystem, EntryData):
    ...
    band_gap = Quantity(
        ...
        a_eln=ELNAnnotation(component=ELNComponentEnum.NumberEditQuantity, defaultDisplayUnit='eV'),
        unit='eV',
        shape=[],
    )
    a_ions = SubSection(
        section_def=PerovskiteIon,
        repeats=True,
    )
    b_ions = SubSection(
        section_def=PerovskiteIon,
